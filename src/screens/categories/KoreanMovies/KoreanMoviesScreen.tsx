// screens/categories/KoreanMovies/KoreanMoviesScreen.tsx
// Màn hình hiển thị danh sách phim Hàn Quốc
...
interface KoreanMoviesScreenProps {
 ...
 */
const KoreanMoviesScreen: React.FC<KoreanMoviesScreenProps> = ({ navigation }) => {
  ...
  const loadMovies = async (pageNum: number = 1, isRefresh: boolean = false) => {
    try {
      ...
      const result = await movieService.getKoreanMovies({ page: pageNum });
      ...
      if (newMovies && newMovies.length > 0) {
        ...
        console.log(`📱 Đã load trang ${pageNum}, tổng: ${pageNum === 1 ? newMovies.length : movies.length + newMovies.length} phim Hàn Quốc`);
      } else {
        if (pageNum === 1) {
          setError('Không thể tải danh sách phim Hàn Quốc');
        }
        setHasMore(false);
      }
    } catch (err) {
      console.error('❌ Lỗi load phim Hàn Quốc:', err);
    ...
    return (
      <LinearGradient colors={colors.gradientStart} style={styles.container}>
        ...
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Đang tải phim Hàn Quốc...</Text>
        </View>
      </LinearGradient>
    );
  }
  ...
  return (
    <LinearGradient colors={colors.gradientStart} style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.title}>🇰🇷 Phim Hàn Quốc</Text>
...
export default KoreanMoviesScreen;