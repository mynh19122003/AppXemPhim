// screens/categories/NewMovies/NewMoviesScreen.tsx
// Màn hình hiển thị danh sách phim mới nhất
...
interface NewMoviesScreenProps {
 ...
 */
const NewMoviesScreen: React.FC<NewMoviesScreenProps> = ({ navigation }) => {
  ...
  const loadMovies = async (pageNum: number = 1, isRefresh: boolean = false) => {
    try {
      ...
      const newMovies = await movieService.getNewMovies(pageNum);
      ...
      if (newMovies && newMovies.length > 0) {
        ...
        // API "phim-moi-cap-nhat" trả về 10 items mỗi trang  
        setHasMore(newMovies.length >= 10);
        setPage(pageNum);
        
        console.log(`📱 Đã load trang ${pageNum}, tổng: ${pageNum === 1 ? newMovies.length : movies.length + newMovies.length} phim mới`);
      } else {
        if (pageNum === 1) {
      ...
      console.error('❌ Lỗi load phim mới:', err);
    ...
    return (
      <LinearGradient colors={colors.gradientStart} style={styles.container}>
        ...
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Đang tải phim mới...</Text>
        </View>
      </LinearGradient>
    );
  }
  ...
  return (
    <LinearGradient colors={colors.gradientStart} style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.title}>🆕 Phim Mới Nhất</Text>
