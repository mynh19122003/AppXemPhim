// screens/categories/SeriesMovies/SeriesMoviesScreen.tsx
// Màn hình hiển thị danh sách phim bộ
...
interface SeriesMoviesScreenProps {
 ...
 */
const SeriesMoviesScreen: React.FC<SeriesMoviesScreenProps> = ({ navigation }) => {
  ...
  const loadMovies = async (pageNum: number = 1, isRefresh: boolean = false) => {
    try {
      ...
      if (newMovies && newMovies.length > 0) {
        ...
        console.log(`📱 Đã load trang ${pageNum}, tổng: ${pageNum === 1 ? newMovies.length : movies.length + newMovies.length} phim bộ`);
      } else {
        if (pageNum === 1) {
      ...
      console.error('❌ Lỗi load phim bộ:', err);
    ...
    return (
      <LinearGradient colors={colors.gradientStart} style={styles.container}>
        ...
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Đang tải phim bộ...</Text>
        </View>
      </LinearGradient>
    );
  }
...
export default SeriesMoviesScreen;