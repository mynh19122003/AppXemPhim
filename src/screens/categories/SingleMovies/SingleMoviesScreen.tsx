// screens/categories/SingleMovies/SingleMoviesScreen.tsx
// Màn hình hiển thị danh sách phim lẻ
...
interface SingleMoviesScreenProps {
 ...
 */
const SingleMoviesScreen: React.FC<SingleMoviesScreenProps> = ({ navigation }) => {
  ...
  const loadMovies = async (pageNum: number = 1, isRefresh: boolean = false) => {
    try {
      ...
      const result = await movieService.getSingleMovies({ page: pageNum });
      ...
      if (newMovies && newMovies.length > 0) {
        ...
        console.log(`📱 Đã load trang ${pageNum}, tổng: ${pageNum === 1 ? newMovies.length : movies.length + newMovies.length} phim lẻ`);
      } else {
        if (pageNum === 1) {
          setError('Không thể tải danh sách phim lẻ');
        }
        setHasMore(false);
      }
    } catch (err) {
      console.error('❌ Lỗi load phim lẻ:', err);
    ...
    return (
      <LinearGradient colors={colors.gradientStart} style={styles.container}>
        ...
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Đang tải phim lẻ...</Text>
        </View>
      </LinearGradient>
    );
  }
...
export default SingleMoviesScreen;